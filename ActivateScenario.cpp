...
constexpr WORD EXTENDED_KEY_MIN = 0x21;
constexpr WORD EXTENDED_KEY_MAX = 0x2E;
constexpr int  ACTION_MOVE      = 2;
constexpr DWORD LOOP_PAUSE_MS   = 100; 
...
void playScenario() {
    ...
    do {
        ...
        while (offset < scenarioBuffer.size() && currentState == ProgramState::Playing) {
            ...
            if (type == EventType::Keyboard) {
                ...
                if (rk.vKey >= EXTENDED_KEY_MIN && rk.vKey <= EXTENDED_KEY_MAX)
    ...
    std::cout << "[PLAY] Finished. Modifier keys released.\n";
}